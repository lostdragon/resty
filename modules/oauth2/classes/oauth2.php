...
class OAuth2 {
	...
	const CONFIG_ACCESS_LIFETIME_BY_LEVEL = "access_token_lifetime_by_level";
	 ...
	 *
	 * @param OAuth2_Storage $storage
	 * @param array $config - An associative array as below of config options. See CONFIG_* constants.
	 */
	public function __construct(OAuth2_Storage $storage, $config = array()) {
	 ...
	 * @param OAuth2_Storage $storage
	 * @param array $config
	 * @return OAuth2 返回实例对象
	 */
	public static function instance(OAuth2_Storage $storage, $config = array())
	{
		if (! isset(OAuth2::$instance))
		{
			// Create a new instance
			OAuth2::$instance = new OAuth2($storage, $config);
		}
	...
	protected function setDefaultOptions() {
		$this->conf = Rest_Config::get('oauth2') + array(
			'client_id_regexp' => self::CLIENT_ID_REGEXP,
			self::CONFIG_ACCESS_LIFETIME => self::DEFAULT_ACCESS_TOKEN_LIFETIME,
			self::CONFIG_ACCESS_LIFETIME_BY_LEVEL => array(86400,604800,2592000,7776000,15552000,31536000,86400000),
	...
	public function verifyAccessToken($token_param, $scope = NULL) {
		...
		if (!$token_param) { // Access token was not provided
			throw new OAuth2_Exception_Authenticate(Rest_Response::HTTP_BAD_REQUEST, $tokenType, $realm, self::ERROR_INVALID_REQUEST, 'The request is missing a required parameter, includes an unsupported parameter or parameter value, repeats the same parameter, uses more than one method for including an access token, or is otherwise malformed.', $scope);
		}
		
		// Get the stored token data (from the implementing subclass)
		$token = $this->storage->getAccessToken($token_param);
		if ($token === NULL) {
			throw new OAuth2_Exception_Authenticate(Rest_Response::HTTP_UNAUTHORIZED, $tokenType, $realm, self::ERROR_INVALID_GRANT, 'The access token provided is invalid.', $scope);
		}
		
		// Check we have a well formed token
		if (!isset($token["expires"]) || !isset($token["client_id"])) {
			throw new OAuth2_Exception_Authenticate(Rest_Response::HTTP_UNAUTHORIZED, $tokenType, $realm, self::ERROR_INVALID_GRANT, 'Malformed token (missing "expires" or "client_id")', $scope);
		}
		
		// Check token expiration (expires is a mandatory paramter)
		if (isset($token["expires"]) && time() > $token["expires"]) {
			throw new OAuth2_Exception_Authenticate(Rest_Response::HTTP_UNAUTHORIZED, $tokenType, $realm, self::ERROR_INVALID_GRANT, 'The access token provided has expired.', $scope);
		}
		
		// Check scope, if provided
		if($scope) {
			if (!isset($token["scope"]) || !$token["scope"]) {
				throw new OAuth2_Exception_Authenticate(Rest_Response::HTTP_FORBIDDEN, $tokenType, $realm, self::ERROR_INSUFFICIENT_SCOPE, 'The request requires higher privileges than provided by the access token.', $scope);
			}
			$this->verifyScope($scope, $token, $tokenType, $realm);
		}
	 ...
	 * @param string $realm
	 * @throws OAuth2_Exception_Authenticate
	 */
	public function verifyScope($scope, $token, $tokenType = NULL, $realm = NULL)
	{
		// If token doesn't have a scope, it's NULL/empty, or it's insufficient, then throw an error
		if (!$this->checkScope($scope, $token["scope"])) {
			$tokenType = is_null($tokenType) ? $this->getVariable(self::CONFIG_TOKEN_TYPE) : $tokenType;
			$realm = is_null($realm) ? $this->getVariable(self::CONFIG_WWW_REALM) : $realm;
			throw new OAuth2_Exception_Authenticate(Rest_Response::HTTP_FORBIDDEN, $tokenType, $realm, self::ERROR_INSUFFICIENT_SCOPE, 'The request requires higher privileges than provided by the access token.', $scope);
		}
	}
	...
	public function getBearerToken() {
		...
		if ($methodsUsed > 1) {
			throw new OAuth2_Exception_Authenticate(Rest_Response::HTTP_BAD_REQUEST, $tokenType, $realm, self::ERROR_INVALID_REQUEST, 'Only one method may be used to authenticate at a time (Auth header, GET or POST).');
		} elseif ($methodsUsed == 0) {
			throw new OAuth2_Exception_Authenticate(Rest_Response::HTTP_BAD_REQUEST, $tokenType, $realm, self::ERROR_INVALID_REQUEST, 'The access token was not found.');
		}
		
		// HEADER: Get the access token from the header
		if (!empty($headers)) {
			if (!preg_match('/' . self::TOKEN_BEARER_HEADER_NAME . '\s(\S+)/', $headers, $matches)) {
				throw new OAuth2_Exception_Authenticate(Rest_Response::HTTP_BAD_REQUEST, $tokenType, $realm, self::ERROR_INVALID_REQUEST, 'Malformed auth header');
			}
		...
		if (isset($_POST[self::TOKEN_PARAM_NAME])) {
			if ($_SERVER['REQUEST_METHOD'] != 'POST') {
				throw new OAuth2_Exception_Authenticate(Rest_Response::HTTP_BAD_REQUEST, $tokenType, $realm, self::ERROR_INVALID_REQUEST, 'When putting the token in the body, the method must be POST.');
			}
			
			// IETF specifies content-type. NB: Not all webservers populate this _SERVER variable
			if (isset($_SERVER['CONTENT_TYPE']) && $_SERVER['CONTENT_TYPE'] != 'application/x-www-form-urlencoded') {
				throw new OAuth2_Exception_Authenticate(Rest_Response::HTTP_BAD_REQUEST, $tokenType, $realm, self::ERROR_INVALID_REQUEST, 'The content type for POST requests must be "application/x-www-form-urlencoded"');
			}
	 ...
	 * @throws OAuth2_Exception_Server
	 * 
	 * @see http://tools.ietf.org/html/draft-ietf-oauth-v2-20#section-4
	...
	public function grantAccessToken(array $inputData = NULL, array $authHeaders = NULL) {
		...
		if (!$input["grant_type"]) {
			throw new OAuth2_Exception_Server(Rest_Response::HTTP_BAD_REQUEST, self::ERROR_INVALID_REQUEST, 'Invalid grant_type parameter or parameter missing');
		}
		...
		if ($this->storage->checkClientCredentials($client[0], $client[1]) === FALSE) {
			throw new OAuth2_Exception_Server(Rest_Response::HTTP_BAD_REQUEST, self::ERROR_INVALID_CLIENT, 'The client credentials are invalid');
		}
		
		if (!$this->storage->checkRestrictedGrantType($client[0], $input["grant_type"])) {
			throw new OAuth2_Exception_Server(Rest_Response::HTTP_BAD_REQUEST, self::ERROR_UNAUTHORIZED_CLIENT, 'The grant type is unauthorized for this client_id');
		}
		
		// Do the granting
		switch ($input["grant_type"]) {
			case self::GRANT_TYPE_AUTH_CODE:
				if (!($this->storage instanceof OAuth2_Interface_Grant_Code)) {
					throw new OAuth2_Exception_Server(Rest_Response::HTTP_BAD_REQUEST, self::ERROR_UNSUPPORTED_GRANT_TYPE);
				}
				
				if (!$input["code"]) {
					throw new OAuth2_Exception_Server(Rest_Response::HTTP_BAD_REQUEST, self::ERROR_INVALID_REQUEST, 'Missing parameter. "code" is required');
				}
				
				if ($this->getVariable(self::CONFIG_ENFORCE_INPUT_REDIRECT) && !$input["redirect_uri"]) {
					throw new OAuth2_Exception_Server(Rest_Response::HTTP_BAD_REQUEST, self::ERROR_INVALID_REQUEST, "The redirect URI parameter is required.");
				}
				...
				if ($stored === NULL || $client[0] != $stored["client_id"]) {
					throw new OAuth2_Exception_Server(Rest_Response::HTTP_BAD_REQUEST, self::ERROR_INVALID_GRANT, "Refresh token doesn't exist or is invalid for the client");
				}
				...
				if ($input["redirect_uri"] && !$this->validateRedirectUri($input["redirect_uri"], $stored["redirect_uri"])) {
					throw new OAuth2_Exception_Server(Rest_Response::HTTP_BAD_REQUEST, self::ERROR_REDIRECT_URI_MISMATCH, "The redirect URI is missing or do not match");
				}
				
				if ($stored["expires"] < time()) {
					throw new OAuth2_Exception_Server(Rest_Response::HTTP_BAD_REQUEST, self::ERROR_INVALID_GRANT, "The authorization code has expired");
				}
				break;
			
			case self::GRANT_TYPE_USER_CREDENTIALS:
				if (!($this->storage instanceof OAuth2_Interface_Grant_User)) {
					throw new OAuth2_Exception_Server(Rest_Response::HTTP_BAD_REQUEST, self::ERROR_UNSUPPORTED_GRANT_TYPE);
				}
				
				if (!$input["username"] || !$input["password"]) {
					throw new OAuth2_Exception_Server(Rest_Response::HTTP_BAD_REQUEST, self::ERROR_INVALID_REQUEST, 'Missing parameters. "username" and "password" required');
				}
				
				$stored = $this->storage->checkUserCredentials($client[0], $input["username"], $input["password"]);
				
				if ($stored === FALSE) {
					throw new OAuth2_Exception_Server(Rest_Response::HTTP_BAD_REQUEST, self::ERROR_INVALID_GRANT, 'Username and password not match');
				}
				break;
			
			case self::GRANT_TYPE_CLIENT_CREDENTIALS:
				if (!($this->storage instanceof OAuth2_Interface_Grant_Client)) {
					throw new OAuth2_Exception_Server(Rest_Response::HTTP_BAD_REQUEST, self::ERROR_UNSUPPORTED_GRANT_TYPE);
				}
				
				if (empty($client[1])) {
					throw new OAuth2_Exception_Server(Rest_Response::HTTP_BAD_REQUEST, self::ERROR_INVALID_CLIENT, 'The client_secret is mandatory for the "client_credentials" grant type');
				}
			...
			case self::GRANT_TYPE_REFRESH_TOKEN:
				if (!($this->storage instanceof OAuth2_Interface_RefreshTokens)) {
					throw new OAuth2_Exception_Server(Rest_Response::HTTP_BAD_REQUEST, self::ERROR_UNSUPPORTED_GRANT_TYPE);
				}
				
				if (!$input["refresh_token"]) {
					throw new OAuth2_Exception_Server(Rest_Response::HTTP_BAD_REQUEST, self::ERROR_INVALID_REQUEST, 'No "refresh_token" parameter found');
				}
				
				$stored = $this->storage->getRefreshToken($input["refresh_token"]);
				
				if ($stored === NULL || $client[0] != $stored["client_id"]) {
					throw new OAuth2_Exception_Server(Rest_Response::HTTP_BAD_REQUEST, self::ERROR_INVALID_GRANT, 'Invalid refresh token');
				}
				
				if ($stored["expires"] < time()) {
					throw new OAuth2_Exception_Server(Rest_Response::HTTP_BAD_REQUEST, self::ERROR_INVALID_GRANT, 'Refresh token has expired');
				}
			...
			case self::GRANT_TYPE_IMPLICIT:
				...
				throw new OAuth2_Exception_Server('501 Not Implemented', 'This OAuth2 library is not yet complete. This functionality is not implemented yet.');
				if (!($this->storage instanceof OAuth2_Interface_Grant_Implicit)) {
					throw new OAuth2_Exception_Server(Rest_Response::HTTP_BAD_REQUEST, self::ERROR_UNSUPPORTED_GRANT_TYPE);
				}
			...
			case filter_var($input["grant_type"], FILTER_VALIDATE_URL):
				if (!($this->storage instanceof OAuth2_Interface_Grant_Extension)) {
					throw new OAuth2_Exception_Server(Rest_Response::HTTP_BAD_REQUEST, self::ERROR_UNSUPPORTED_GRANT_TYPE);
				}
				...
				if ($stored === FALSE) {
					throw new OAuth2_Exception_Server(Rest_Response::HTTP_BAD_REQUEST, self::ERROR_INVALID_GRANT);
				}
				break;
		...
		if ($input["scope"] && (!is_array($stored) || !isset($stored["scope"]) || !$this->checkScope($input["scope"], $stored["scope"]))) {
			throw new OAuth2_Exception_Server(Rest_Response::HTTP_BAD_REQUEST, self::ERROR_INVALID_SCOPE, 'An unsupported scope was requested.');
		}
		
		$user_id = isset($stored['user_id']) ? $stored['user_id'] : null;
		$token = $this->createAccessToken($client[0], $user_id, $stored['scope'], $input["grant_type"]);
	...
	protected function getClientCredentials(array $inputData, array $authHeaders) {
		...
		if (!empty($authHeaders['PHP_AUTH_USER'])) {
			...
			throw new OAuth2_Exception_Server(Rest_Response::HTTP_BAD_REQUEST, self::ERROR_INVALID_CLIENT, 'Client id was not found in the headers or body');
		} elseif (empty($inputData['client_secret'])) { // No credentials were specified
			throw new OAuth2_Exception_Server(Rest_Response::HTTP_BAD_REQUEST, self::ERROR_INVALID_CLIENT, 'Client secret was not found in the headers or body');
		} else {
	...
	public function getAuthorizeParams(array $inputData = NULL) {
		$filters = array(
			"client_id" => array("filter" => FILTER_VALIDATE_REGEXP, "options" => array("regexp" => $this->getVariable('client_id_regexp')), "flags" => FILTER_REQUIRE_SCALAR),
		...
		if (!$input["client_id"]) {
			throw new OAuth2_Exception_Server(Rest_Response::HTTP_BAD_REQUEST, self::ERROR_INVALID_CLIENT, "No client id supplied"); // We don't have a good URI to use
		}
		
		// Get client details
		$stored = $this->storage->getClientDetails($input["client_id"]);
		if ($stored === FALSE) {
			throw new OAuth2_Exception_Server(Rest_Response::HTTP_BAD_REQUEST, self::ERROR_INVALID_CLIENT, "Client id does not exist");
		}
		...
		if (!$input["redirect_uri"] && !$stored["redirect_uri"]) {
			throw new OAuth2_Exception_Server(Rest_Response::HTTP_BAD_REQUEST, self::ERROR_REDIRECT_URI_MISMATCH, 'No redirect URL was supplied or stored.');
		}
		if ($this->getVariable(self::CONFIG_ENFORCE_INPUT_REDIRECT) && !$input["redirect_uri"]) {
			throw new OAuth2_Exception_Server(Rest_Response::HTTP_BAD_REQUEST, self::ERROR_REDIRECT_URI_MISMATCH, 'The redirect URI is mandatory and was not supplied.');
		}
		...
		if ($stored["redirect_uri"] && $input["redirect_uri"] && !$this->validateRedirectUri($input["redirect_uri"], $stored["redirect_uri"])) {
			throw new OAuth2_Exception_Server(Rest_Response::HTTP_BAD_REQUEST, self::ERROR_REDIRECT_URI_MISMATCH, 'The redirect URI provided is missing or does not match');
		}
		...
		if (!$input["response_type"]) {
			throw new OAuth2_Exception_Redirect($input["redirect_uri"], self::ERROR_INVALID_REQUEST, 'Invalid or missing response type.', $input["state"]);
		}

		if ($input['response_type'] != self::RESPONSE_TYPE_AUTH_CODE && $input['response_type'] != self::RESPONSE_TYPE_ACCESS_TOKEN) {
			throw new OAuth2_Exception_Redirect($input["redirect_uri"], self::ERROR_UNSUPPORTED_RESPONSE_TYPE, NULL, $input["state"]);
		}

		// Validate that the requested scope is supported
		if ($input["scope"] && !$this->checkScope($input["scope"], $this->getVariable(self::CONFIG_SUPPORTED_SCOPES))) {
			throw new OAuth2_Exception_Redirect($input["redirect_uri"], self::ERROR_INVALID_SCOPE, 'An unsupported scope was requested.', $input["state"]);
		}
		
		// Validate state parameter exists (if configured to enforce this)
		if ($this->getVariable(self::CONFIG_ENFORCE_STATE) && !$input["state"]) {
			throw new OAuth2_Exception_Redirect($input["redirect_uri"], self::ERROR_INVALID_REQUEST, "The state parameter is required.");
		}
	...
	public function getAuthResult($is_authorized, $user_id = NULL, $params = array()) {
		...
		$scope = $state = $response_type = $redirect_uri = $client_id = NULL;
		$result = array();
		...
		if ($is_authorized === FALSE) {
			throw new OAuth2_Exception_Redirect($redirect_uri, self::ERROR_USER_DENIED,
				"The user denied access to your application",
				$state,
				$response_type == self::RESPONSE_TYPE_ACCESS_TOKEN ? TRUE : FALSE);
		} else {
			if ($response_type == self::RESPONSE_TYPE_AUTH_CODE) {
				$result["query"]["code"] = $this->createAuthCode($client_id, $user_id, $redirect_uri, $scope);
			} elseif ($response_type == self::RESPONSE_TYPE_ACCESS_TOKEN) {
				/**
				 * @see http://tools.ietf.org/html/draft-ietf-oauth-v2-31#section-9
				 ...
				 */
				$result["fragment"] = $this->createAccessToken($client_id, $user_id, $scope, $response_type);
			}
		}
	...
	private function doRedirectUriCallback($redirect_uri, $params) {
		header("HTTP/1.1 " . Rest_Response::HTTP_FOUND);
	 ...
	 */
	public function createAccessToken($client_id, $user_id, $scope = NULL, $grant_type = NULL,$client_level = 0, $platform_id=0, $device_id="", $phone_model="", $os="") {
		
		$cfg_expire_arr = $this->getVariable(self::CONFIG_ACCESS_LIFETIME_BY_LEVEL);
		$token = array(
			"access_token" => $this->genAccessToken(),
			"expires_in" => $cfg_expire_arr[(int)$client_level],
			"token_type" => $this->getVariable(self::CONFIG_TOKEN_TYPE),
			"scope" => $scope
		);

		//为token添加device_id信息，保证在同一用户在同一设备上可以使用同一队列
		$token['access_token'] .= substr($device_id?$device_id:"",-7);
		
		$this->storage->setAccessToken($token["access_token"], $client_id, $user_id, time() + $token['expires_in'], $scope, $platform_id, $device_id, $phone_model, $os);
		
		// Issue a refresh token also, if we support them
		if ($grant_type === self::GRANT_TYPE_AUTH_CODE && ($this->storage instanceof OAuth2_Interface_RefreshTokens)) {
			$token["refresh_token"] = $this->genAccessToken();
			$this->storage->setRefreshToken($token["refresh_token"], $client_id, $user_id, time() + $this->getVariable(self::CONFIG_REFRESH_LIFETIME), $scope, $platform_id, $device_id, $phone_model, $os);
	...
	protected function genAccessToken() {
		$tokenLen = 40;
		if (is_file('/dev/urandom')) { // Get 100 bytes of random data
