...
$(document).ready(function () {
    ...
    if(access_token) {
        var url = location.href.substr(0, location.href.indexOf('_tools') - 1);
        var id = "72d6ab50-4e45-d2ee-0e79-93395756ec7b";
        var date = new Date().getTime();

        setTimeout(function() {
            var env = pm.envManager.getEnvironmentFromId(id);

            var environment = '{"id":"' + id + '","name":"developer","values":[{"key":"url","value":"' + url + '"},{"key":"access_token","value":"' + access_token + '"}],"timestamp":'+ date +'}'
            environment = JSON.parse(environment);
            if(!env) {
                pm.indexedDB.environments.addEnvironment(environment, function () {
                    pm.envManager.getAllEnvironments();
                    pm.envManager.selectedEnv = environment;
                    pm.settings.set("selectedEnvironmentId", id);
                    pm.envManager.quicklook.refreshEnvironment(environment);
                    $('#environment-selector .environment-list-item-selected').html(environment.name);
                });
            } else {
                if(getEnvironmentAccessToken(env) != access_token) {
                    pm.indexedDB.environments.updateEnvironment(environment, function () {
                        pm.envManager.getAllEnvironments();
                    });
                }
...
function getEnvironmentAccessToken(env) {
    for(var i in env.values) {
        if(env.values[i].key == 'access_token') {
